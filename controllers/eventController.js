...
async function createEvent ({ userID, organizationID, eventName, description }){
    try {
        // Check Access to Organization
        const userorganization = await UserOrganization.findOne({ "userID": userID, "organizationID": organizationID});
        if(!userorganization)
...
async function deleteEvent ({ userID, eventID }){
    try {
        ...
        const userorganization = await UserOrganization.findOne({ "userID": userID, "organizationID": event.organizationID });
        console.log({ "userID": userID, "organizationID": event.organizationID })
        if(!userorganization)
